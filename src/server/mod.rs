...
impl Server {
    ...
    async fn create_mint_event(&self, path: &Path) -> Result<path_item::MintEventResponse> {
        ...
        if next_index > 0 {
            ...
            return Ok(path_item::MintEventResponse {
                filename: file_name,
                sha256: sha256_hex,
        ...
        Ok(path_item::MintEventResponse {
            filename: file_name,
            sha256: sha256_hex.clone(),
            ots_base64: ots_proof_b64,
            event_hash: event_hash_hex,
            issued_at,
            stamp_status: Some(path_item::StampStatus {
                success: false,
                results: None,
