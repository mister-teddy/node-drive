 ...
 */
export async function fetchStatus(url: string, options?: RequestInit): Promise<boolean> {
  const response = await fetch(url, options);
  return response.ok;
}
 ...
 */
export async function fetchMutation(url: string, options?: RequestInit): Promise<void> {
  const response = await fetch(url, options);
