 ...
 * }} props
 */
export default function Provenance({ fileName, defaultMode = "full", isDir = false }) {
  ...
  const isPending = stampStatus !== 'verified';
  ...
  const fetchProvenanceData = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setStampStatus(await getStampStatus({
          detachedOts: manifestData.artifact?.ots_b64 || null,
          detached: manifestData.artifact?.sha256_hex || null
        }));
      }
    } catch (error) {
    ...
    return createElement(
      ...
      createElement(
        ...
        createElement(
          ...
          !!manifest.artifact?.sha256_hex && createElement(
            "kbd",
            {
              style: {
                fontSize: "9px",
    ...
    return createElement(
      ...
      manifest
        ? createElement(
          ...
          createElement(
            "a",
            { style: { marginBottom: "4px" }, href: `${window.location.pathname}${fileName}?manifest=json`, target: "_blank", rel: "noopener noreferrer" },
  ...
  return createElement(
    ...
    isExpanded && manifest ? createElement(
      "div",
      {
        style: {
          position: "fixed",
